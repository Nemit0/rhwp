...
test('Electron bridge는 native dialog로 파일을 열고 같은 경로 또는 새 경로에 저장한다', async () => {
  const writes: Array<{ path: string; text: string }> = [];
  const electronAPI: ElectronFileApi = {
    async openFile() {
      return { filePath: '/tmp/opened.hwp', fileName: 'opened.hwp', data: [97, 98, 99] };
    },
    async readFile(filePath) {
      assert.equal(filePath, '/tmp/opened.hwp');
      return { filePath, fileName: 'opened.hwp', data: [97, 98, 99] };
    },
    async saveFile(filePath, data) {
      writes.push({ path: filePath, text: new TextDecoder().decode(data) });
      return true;
    },
    async saveFileAs(_suggestedName, data) {
      writes.push({ path: '/tmp/copied.hwpx', text: new TextDecoder().decode(data) });
      return { filePath: '/tmp/copied.hwpx', fileName: 'copied.hwpx' };
    },
  };
  const windowLike = { electronAPI };
  const handle = await pickOpenFileHandle(windowLike);

  assert.ok(handle);
  assert.deepEqual(Array.from((await readFileFromHandle(handle)).bytes), [97, 98, 99]);
  ...
  const savedAs = await saveDocumentToFileSystem({
    blob: new Blob(['copy']),
    suggestedName: 'copied.hwpx',
    currentHandle: handle,
    forceSaveAs: true,
    saveFormat: 'hwpx',
    windowLike,
  ...
  assert.deepEqual(writes, [
    { path: '/tmp/opened.hwp', text: 'same' },
    { path: '/tmp/copied.hwpx', text: 'copy' },
  ]);
  assert.equal(savedAs.fileName, 'copied.hwpx');
});
